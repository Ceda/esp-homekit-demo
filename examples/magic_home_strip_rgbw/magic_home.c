...
void hsi2rgb(float h, float s, float i, rgb_color_t *rgb) {
    while (h < 0) {
        h += 360.0F;
    }
    while (h >= 360) {
    ...
    if (h < 2.094393334) {
        r = 1 * LED_RGB_SCALE * i / 3 * (1 + s * cos(h) / cos(1.047196667 - h));
        g = 1 * LED_RGB_SCALE * i / 3 * (1 + s * (1 - cos(h) / cos(1.047196667 - h)));
        b = 1 * LED_RGB_SCALE * i / 3 * (1 - s);
    } else if (h < 4.188786668) {
        h = h - 2.094393334;
        g = 1 * LED_RGB_SCALE * i / 3 * (1 + s * cos(h) / cos(1.047196667 - h));
        b = 1 * LED_RGB_SCALE * i / 3 * (1 + s * (1 - cos(h) / cos(1.047196667 - h)));
        r = 1 * LED_RGB_SCALE * i / 3 * (1 - s);
    } else {
        h = h - 4.188786668;
        b = 1 * LED_RGB_SCALE * i / 3 * (1 + s * cos(h) / cos(1.047196667 - h));
        r = 1 * LED_RGB_SCALE * i / 3 * (1 + s * (1 - cos(h) / cos(1.047196667 - h)));
        g = 1 * LED_RGB_SCALE * i / 3 * (1 - s);
    }
    const uint32_t w = 1 * LED_RGB_SCALE * i * (1 - s);

    rgb->red = (uint8_t) ((r > LED_RGB_SCALE) ? LED_RGB_SCALE : r);
    rgb->green = (uint8_t) ((g > LED_RGB_SCALE) ? LED_RGB_SCALE : g);
    rgb->blue = (uint8_t) ((b > LED_RGB_SCALE) ? LED_RGB_SCALE : b);
    rgb->white = (uint8_t) ((w > LED_RGB_SCALE) ? LED_RGB_SCALE : w);
}
...
void create_accessory_name() {
        ...
        int name_len = snprintf(NULL, 0, "LED Strip %02X:%02X:%02X",
                                macaddr[3], macaddr[4], macaddr[5]);
        char *name_value = malloc(name_len+1);
        snprintf(name_value, name_len+1, "LED Strip %02X:%02X:%02X",
                 macaddr[3], macaddr[4], macaddr[5]);
